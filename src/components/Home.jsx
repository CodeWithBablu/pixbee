import {
  ...
  ModalFooter,
  ModalOverlay,
} from "@chakra-ui/react"; //// Componets required from chakra
...
const instaicon = (
  <svg
    width="15"
    height="15"
    className="mr-1 stroke-slate-600"
    viewBox="0 0 15 15"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
  >
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const res = await fetch(
        `https://api.unsplash.com/photos?per_page=30&client_id=${process.env.ACCESS_KEY}`
      );
        ...
        <div>
          <div className="  pt-20">
            <div className=" relative w-full h-[384px]">
              ...
              <div className=" absolute top-28 left-0 right-0 mx-auto flex flex-col flex-grow max-w-[600px] px-6 space-y-2">
                <span className=" text-white text-center text-md sm:text-xl font-poppins font-semibold">
                  Download High Quality Images by creators
                </span>
                <span className=" text-center text-sm text-slate-700 font-semibold">
                  Over 2.4 millions stocks images by our community
                </span>
                <Search
                  text={"Search high resolution images"}
                  allowMaxWidth={false}
                />
              </div>
            </div>
          ...
          <div className=" mt-20 px-2 w-full columns-1 sm:columns-2 md:columns-3 xl:columns-4 gap-4 space-y-3">
            ...
            {data.images &&
              data.images.map((photo) => (
                <div
                  ...
                  className=" cursor-pointer flex flex-col rounded-xl border-[1px] border-slate-300 dark:bg-primary dark:border-0 overflow-hidden"
                >
                  <img
                  ...
                  <div className=" flex items-center justify-between px-2 py-4">
                    <div className=" flex space-x-2">
                      ...
                      <div className=" grid grid-rows-2">
                        <span className=" text-secondary dark:text-white text-[13px] font-poppins font-semibold">
                          {photo.user.name}
                        </span>
                        <span className=" text-slate-500 dark:text-dimWhite text-[10px] font-dynapuff font-bold">
                          @{photo.user.username}
                        </span>
                      </div>
                    </div>

                    <span className=" flex items-center text-[14px] font-semibold dark:text-white">
                      {likeicon}
                      {photo.likes >= 1000
                        ? `${photo.likes / 1000}`
                        : `${photo.likes}`}
                      {photo.likes >= 1000 && <span>k</span>}
                    </span>
                  </div>
                </div>
              ))}
            ...
            <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)}>
              ...
              <ModalContent
                borderRadius="15px"
                className=" w-full md:min-w-[645px] xl:min-w-[945px] rounded-3xl"
              >
                <ModalBody
                  padding={0}
                  margin={0}
                  className=" relative rounded-t-[15px]"
                >
                  <span
                    onClick={() => setModalOpen(false)}
                    className=" cursor-pointer absolute -top-3 right-0 md:-right-3 rounded-full p-2 bg-white"
                  >
                    {closeicon}
                  </span>
                  ...
                  <div className=" absolute w-full px-2 bottom-2 flex justify-between items-center">
                    <div className=" grid grid-cols-2 gap-x-2">
                      <span className=" cursor-pointer p-1 border-2 border-white rounded-md text-white font-semibold flex  items-center">
                        {shareicon}Share
                      </span>
                      <span className=" cursor-pointer p-1 border-2 border-white rounded-md text-white font-semibold flex  items-center">
                        {infoicon}info
                    ...
                    <span className=" cursor-pointer bg-teal-400 text-secondary text-xl px-4 py-2 rounded-lg font-semibold font-dynapuff">
                      Download
                    </span>
                  </div>
                  ...
                  <img
                    ...
                    src={modalData.urls ? modalData.urls.regular : ""}
                    alt=""
                  />
                </ModalBody>
                        ...
                        <div className=" flex flex-grow items-center justify-between ">
                          <div className=" grid grid-cols-2 gap-x-2">
                            {modalData.user.instagram_username && (
                              <span className=" cursor-pointer flex items-center text-xs font-dynapuff text-slate-600">
                                {instaicon} /{" "}
                                {modalData.user.instagram_username}
                              </span>
                            )}
                          ...
                          <span className=" flex items-center text-[14px] font-semibold dark:text-white">
                            ...
                            {modalData.likes >= 1000
                              ? `${modalData.likes / 1000}`
                              : `${modalData.likes}`}
                            {modalData.likes >= 1000 && <span>k</span>}
                          </span>
                        </div>
                      </div>
                    </div>
